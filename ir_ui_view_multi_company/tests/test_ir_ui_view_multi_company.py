...
class TestIrUiViewMultiCompany(TransactionCase):
    ...
    `base.view_view_form`
    to include a `company_id` field in the form view. In this test class we test
    ir.ui.view form view to see how the new field is displayed if we change the
    view company

    - The `company_id` field is always visible if the view is not linked to any company.
    ...
    def _is_company_field_is_in_form_view(self, company):
        ...
        return bool(tree.xpath('//field[@name="company_id"]'))

    def test_0(self):
        """
        If the view is not linked to any company (shared view), the 'company_id' field
    ...
    def test_1(self):
        ...
        """
        self.view.company_id = self.company_2
